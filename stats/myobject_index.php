...
/**
 *	    \file       mfa/stats/myobject_index.php
 *      \ingroup    order
...
dol_include_once('/mfa/class/myobject.class.php');
dol_include_once('/mfa/lib/mfa_myobject.lib.php');
...
$hookmanager->initHooks(array('mfastats', 'myobjectstats', 'globalcard'));
...
$langs->loadLangs(array('companies', 'other', 'mfa@mfa'));
...
$newcardbutton .= dolGetButtonTitle($langs->trans('Statistics'), '', 'fa fa-chart-bar imgforviewmode', dol_buildpath('/mfa/stats/index.php', 1).'?objecttype=myobject'.preg_replace('/(&|\?)*(mode|groupby)=[^&]+/', '', $param), '', ($mode == 'statistics' ? 2 : 1), array('morecss' => 'reposition'));
$newcardbutton .= dolGetButtonTitleSeparator();
$newcardbutton .= dolGetButtonTitle($langs->trans('New'), '', 'fa fa-plus-circle', dol_buildpath('/mfa/myobject_card.php', 1).'?action=create&backtopage='.urlencode($_SERVER['PHP_SELF']), '', $permissiontoadd);
